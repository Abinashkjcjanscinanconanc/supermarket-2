    ...
    private void rSButtonHover1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_rSButtonHover1ActionPerformed
           if(com.getSelectedItem().toString().equals("SELLER"))
           {
            try{
                ...
                String q="select * from seller where sellername='"+uID.getText()+"' and sellerpass='"+uPassword.getText()+"'";
            ...
            }catch(SQLException se)
            {
               ...
               try{
                cn=DriverManager.getConnection("jdbc:mysql://localhost:3306/Super_Market","root","root");
                st=cn.createStatement();
                String q="select * from admin where adminname='"+uID.getText()+"' and adminpass='"+uPassword.getText()+"'";
                rs=st.executeQuery(q);
                if(rs.next())
                {
                    setVisible(false);
